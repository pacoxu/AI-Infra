...
const dataPath = path.join(radarRoot, 'mcm-multicluster-radar.data.json');
const outEnPath = path.join(radarRoot, 'mcm-multicluster-radar.svg');
const outZhPath = path.join(radarRoot, 'mcm-multicluster-radar.zh-CN.svg');
...
const localeMap = {
  en: {
    ...
    maintenance:
      'Maintenance: edit archive/mcm-radar/mcm-multicluster-radar.data.json and run node archive/mcm-radar/scripts/generate-mcm-radar-svg.js',
    scope:
  ...
  'zh-CN': {
    ...
    maintenance:
      '维护：编辑 archive/mcm-radar/mcm-multicluster-radar.data.json，然后运行 node archive/mcm-radar/scripts/generate-mcm-radar-svg.js',
...
function logoDataUri(logoPath) {
  ...
  const absolutePath = path.isAbsolute(logoPath) ? logoPath : path.join(radarRoot, logoPath);
...
console.log(
  `Generated ${path.relative(radarRoot, outEnPath)} and ${path.relative(radarRoot, outZhPath)} from ${path.relative(radarRoot, dataPath)}`
);
