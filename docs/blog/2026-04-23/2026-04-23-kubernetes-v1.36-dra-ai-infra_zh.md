...
2026-05-08 更新：根据 2026-05-07 官方 DRA blog，补充了近期 `v1.33 -> v1.34 -> v1.36`
的演进线、`Deterministic Device Selection` 小节，并把 `Device Taints and Tolerations`
在 `v1.36` 中的阶段表述修正为 Beta。
...
这里也有一个容易混淆的历史脉络。`KEP-3063` 可以看作之前的 classic DRA 路线，它定义了
早期 DRA 控制面与调度协作的大框架，但后来停在 withdrawn 状态。今天 Kubernetes 里真正
落地并稳定下来的 core API 主线，更接近 `KEP-4381 DRA Structured Parameters`：它把
当前的 `ResourceClaim`、`ResourceSlice`、结构化参数与调度协作方式继续推进，并在
v1.34 进入 Stable。后面很多 v1.36 和 v1.37 的 DRA KEP，本质上都是沿着这条主线继续补
共享、拓扑、状态、治理和规模能力。
...
## 结合近期官方 DRA blog 看演进线

如果把最近几篇官方 DRA blog 放在一起看，DRA 的演进线会比单看 `v1.36` release blog
更清楚：

| 版本 | 关键信号 | 对 AI-Infra 的意义 |
| --- | --- | --- |
| [v1.33 DRA updates](https://kubernetes.io/blog/2025/05/01/kubernetes-v1-33-dra-updates/) | DRA 还处在“核心 API 持续打磨、运维与调度能力继续孵化”的阶段。 | 更像是在把 claim/status/admin 这些基础拼图补齐。 |
| [v1.34 DRA gets even more powerful](https://kubernetes.io/blog/2025/09/01/kubernetes-v1-34-dra-updates/) | DRA 开始明显转向“可生产迁移”的叙事，重点落在迁移、可观测、共享与健康。 | 这是从 Device Plugin 向 DRA 渐进迁移第一次变得现实的节点。 |
| [v1.36: More Drivers, New Features, and the Next Era of DRA](https://kubernetes.io/blog/2026/05/07/kubernetes-v1-36-dra-136-updates/) | 关注点从“把 DRA 做扎实”进一步扩展到“让更多驱动和更多资源类型进入同一模型”。 | DRA 不再只是在谈 GPU，而是在向 NIC、CPU、memory、PodGroup 和容量可见性延伸。 |

这条时间线也解释了为什么本文会同时讨论两类东西：

- 一类是官方 2026-05-07 blog 直接点名的 `v1.36` 用户可感知 feature；
- 另一类是 AdminAccess、PodResources API、ResourceClaim Device Status、
  Consumable Capacity 这类虽然不一定都算“v1.36 新 feature”，但对理解 DRA 为何开始
 具备生产闭环非常关键的底座能力。

## v1.36 的 KEP 版图与当前完整度

如果按 2026-05-07 官方 DRA blog 的“用户视角 feature map”来看，`v1.36` 主要可以读成
**1 个 Stable、5 个 Beta、6 个 Alpha**。这是一种更贴近“本版本用户会直接感知到什么”的
分类方式：

| 阶段 | 能力 | AI-Infra 价值 |
| --- | --- | --- |
| Stable | Prioritized List | 稀缺设备请求支持优先级备选。 |
| Beta | Extended Resource Support | 传统 extended resource 请求可渐进迁移到 DRA 后端。 |
| Beta | Partitionable Devices | 表达动态分区和互斥分区关系。 |
| Beta | Device Taints and Tolerations | 单设备维护、健康退化和调度隔离。 |
| Beta | Device Binding Conditions | 等外部设备 attach/prepare 后再绑定 Pod。 |
| Beta | Resource Health Status | Pod status 暴露已分配设备健康状态。 |
| Alpha | ResourceClaim Support for Workloads | PodGroup / Workload 级别共享 `ResourceClaim`。 |
| Alpha | DRA Node Allocatable Resources | 探索用 DRA 管理 CPU、memory 等 node allocatable 资源。 |
| Alpha | Resource Availability Visibility | 按需查询资源池可用性。 |
| Alpha | List Types for Attributes | 设备属性支持 list 类型和集合语义。 |
| Alpha | Deterministic Device Selection | 让驱动能通过稳定排序更主动影响最终设备选择。 |
| Alpha | Device Metadata in Containers | 通过标准文件协议把设备 metadata 暴露给容器。 |

但如果按本文的“控制面完整度”视角来看，还需要把几块前一阶段逐步成熟、但对 `v1.36`
实际落地很关键的底座能力一并纳入：
...
两张表合起来看，更接近 `v1.36` 的真实含义：它既有一批用户直接可见的新特性和阶段提升，
也建立在前一阶段已经逐渐稳定下来的 claim status、节点可观测、管理员访问和共享语义之上。
下面不按 KEP 序号罗列，而是按问题域展开。
...
官方 2026-05-07 DRA blog 里新增、而且很容易被忽略的一点，是
`Deterministic Device Selection`。它做的不是再引入一种新约束，而是把 scheduler
评估设备时的顺序变得可预测：基于资源池和 `ResourceSlice` 名称的字典序来评估。

这件事的价值在于，驱动作者终于可以更稳定地影响“先看哪些设备、后看哪些设备”：

- 可以让更优先的 GPU/NIC/fabric 组合先被尝试；
- 可以让资源池内部的顺序更接近 driver 作者预期的拓扑或性能排序；
- 可以减少“同样的输入却因为遍历顺序变化拿到不同结果”的随机感。

但这条能力也有边界。它更接近“稳定化 first-fit 的设备遍历顺序”，而不是完整的
device scoring 机制。也就是说，它能改善可预测性，帮助驱动更主动地组织资源池，
但还不能替代 `#4970` 这类更彻底的 device/node scoring 设计。
...
不过这条线虽然在 v1.36 已进入 Beta，但从平台落地体验看仍然偏早期。它目前没有配套的
`kubectl` 管理体验，toleration 是定义
...
- **中期值得验证**：围绕 Partitionable Devices、Consumable Capacity、Prioritized List
  和 device taints 提升 GPU/MIG/网络资源池利用率与维护粒度；
- **长期需要继续观察**：Workload 级 claim、deterministic selection 之外的完整 scoring、
  node allocatable 统一账本、资源池可见性和容器内 metadata 这些方向，仍然需要更多实现
  经验来证明它们在复杂生产环境中的可操作性。

## 参考

- Kubernetes v1.36 release blog:
  <https://kubernetes.io/blog/2026/04/22/kubernetes-v1-36-release/>
- Kubernetes v1.36 DRA blog:
  <https://kubernetes.io/blog/2026/05/07/kubernetes-v1-36-dra-136-updates/>
- Kubernetes v1.34 DRA blog:
  <https://kubernetes.io/blog/2025/09/01/kubernetes-v1-34-dra-updates/>
- Kubernetes v1.33 DRA blog:
  <https://kubernetes.io/blog/2025/05/01/kubernetes-v1-33-dra-updates/>
...
- KEP-4381 DRA Structured Parameters:
  <https://github.com/kubernetes/enhancements/issues/4381>
- KEP-3063 dynamic resource allocation (classic DRA, withdrawn):
  <https://github.com/kubernetes/enhancements/issues/3063>
