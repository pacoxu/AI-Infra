...
# Kubernetes v1.36 DRA：异构资源调度正在变成完整控制面
...
Dynamic Resource Allocation，简称 DRA，就是 Kubernetes 为这些问题设计的下一代
异构资源管理框架。它不是 Device Plugin 的简单替代品，而是把设备资源从
“节点上报一个整数”推进到“API 描述、scheduler 决策、kubelet 准备、runtime 注入、
状态面观测、管理员治理”的完整控制面。
...
**DRA 的核心抽象已经越过“能表达设备”的阶段，开始补齐生产环境真正需要的迁移、
共享、健康、异步绑定、可观测和管理能力。**

对 AI-Infra 团队而言，最值得关注的是五件事：

1. **兼容迁移**：DRA Extended Resource 让传统 extended resource 请求可以映射到
   DRA 后端，降低存量 GPU workload 迁移成本。
2. **真实硬件建模**：Partitionable Devices、Consumable Capacity 和 list attributes
   让 scheduler 能理解 MIG、共享 capacity、拓扑标签集合等复杂资源形态。
3. **调度可靠性**：Prioritized List、Device Binding Conditions 和 device taints
   让稀缺资源降级、异步 attach、单设备维护进入标准调度流程。
4. **状态可观测**：ResourceClaim device status、Pod resource health 和 PodResources
   API 让设备分配结果、健康状态和节点侧用量变成 Kubernetes 原生事实。
5. **运维边界**：AdminAccess 给管理员诊断和设备管理提供受控入口，不需要把租户权限
   或节点权限粗暴放大。
...
| Beta | KEP-5055 | Device Taints and Tolerations | 单设备维护、健康退化和调度隔离 |
| Beta | KEP-5007 | Device Binding Conditions | 等外部设备 attach/prepare 后再绑定 Pod |
| Beta | KEP-4817 | ResourceClaim Device Status | driver 在 claim status 中报告分配后状态 |
...
这张表背后的信号很明确：v1.36 不是单点增强，而是在围绕 DRA 核心 API 补生产能力。
下面按问题域展开。
...
真正落地时，平台团队需要重点设计资源命名、节点池隔离和配额策略。
最重要的是避免同一块物理设备同时被 Device Plugin 和 DRA driver 管理。
...
可以成为 EndpointSlice、Service、gateway controller、telemetry agent 的输入。
...
也容易引入权限扩大和竞态。Device Metadata in Containers 的方向，是通过 CDI 和
downward API 风格机制，把与本容器相关的设备 metadata 以文件形式挂载进去。
...
### 第三阶段：把健康、taint 和状态闭环接起来

DRA 真正生产化，不在于 claim 能创建成功，而在于异常时系统是否可解释、可恢复：

- 设备异常是否进入 Pod status；
- 单设备是否能被 taint，而不是必须 drain node；
...
### Driver 成熟度比 API 阶段更重要

某个 KEP 进入 Beta 或 Stable，不代表所有厂商 driver 已经同等成熟。生产评估时要看：

- driver 支持哪些 API 字段；
- 是否支持 CDI；
- 是否支持 prepare/unprepare 幂等；
- 节点重启、kubelet 重启、driver 重启后状态能否恢复；
- 与现有 Device Plugin、GPU Operator、CNI、NRI 是否冲突。

### DRA 与 Device Plugin 不能无边界共管同一设备

迁移期很容易出现同一块 GPU 同时被两个系统看到的情况。平台要么按节点池隔离，要么由
driver 明确提供兼容层，不能依赖“调度时刚好不冲突”。

### 调度成功不等于性能达标

DRA 能表达拓扑和 capacity，但性能仍取决于真实硬件路径、driver 实现、runtime 注入、
网络配置和应用初始化。GPU + NIC 同 PCIe root、同 NUMA node、同 fabric domain 只是
基础条件，NCCL、RDMA、CNI 和内核参数还需要独立验证。

### 可观测性要先于大规模上线

没有 claim status、Pod resource health、PodResources API 和设备 telemetry 的关联，
DRA 出问题时会更难排查。建议把可观测性作为上线门槛，而不是上线后的补项。

## 结语

Kubernetes v1.36 的 DRA 版图说明，DRA 已经不只是“更灵活的 GPU 请求 API”。它正在把
异构设备管理变成 Kubernetes 原生资源控制面：
...
短期看，最务实的切入点是 DRA Extended Resource、ResourceClaim 状态、设备健康和
单设备 taint。中期看，Partitionable Devices、Consumable Capacity 和 Prioritized List
会决定 GPU/MIG/网络资源池的利用率。长期看，Workload 级 claim、node allocatable 统一
账本和资源池可见性，才是大规模 AI 集群真正需要的资源控制面。
