...
### 1) 用 VPA 建议值与就地扩缩能力降低恢复抖动

当冷启动本身难以继续压缩时，可先用 VPA 把 CPU/内存请求收敛到更合理区间，减少恢复阶段因为资源不足导致的连锁抖动。注意 VPA 与“Pod 就地垂直扩缩”不是同一个机制，VPA 在部分模式下可能触发驱逐重建。建议先从 `Off` 或 `Initial` 模式观察建议值，再逐步放开自动化策略。

### 2) 用 Kubernetes v1.35 的“就地重启全部容器”

`RestartAllContainers`（v1.35 alpha）在上游 Kubernetes 中是由容器退出事件按重启策略触发的动作（见 KEP-5532），并不是任意时刻可手动调用的“重启全部容器 API”。它适用于“需要重跑 init 或重置状态，但不想整 Pod 重新调度”的场景。前提是集群开启并正确配置相关特性门控和退出触发策略；满足这些条件时，往往比删除重建更快，尤其在大规模批处理和同步作业里价值明显。

### 3) 让就绪检查与门禁表达“业务可服务”

优先将模型加载状态集成到容器 `readinessProbe` 中。对于需要外部协同的场景，再利用 `readinessGates` 表达 `ModelLoaded/WarmupDone` 状态，避免实例在“容器 Ready 但模型未热”时提前接流量。这一点在混部和灰度发布中效果非常直接。
