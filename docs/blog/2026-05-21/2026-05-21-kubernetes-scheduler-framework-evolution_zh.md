...
source_urls:
  ...
  - https://v1-35.docs.kubernetes.io/docs/concepts/scheduling-eviction/gang-scheduling/
...
## 3. framework 稳住之后，scheduler 开始不只是“选节点”

framework 稳定之后，后面的演进不再是“是否有插件架子”，而是这个架子开始承载更多层次的问题。

### 3.1 从单一默认链路，走向 `profiles`
...
- `QueueingHint` 在官方文档里已经是 **`v1.34 stable`**，而 Kubernetes 官方博客把它作为 `v1.32` 的 scheduler 吞吐优化重点来介绍。
...
4. **framework 继续长大**  
   `profiles`、`multiPoint`、`PreEnqueue`、`QueueingHint`，说明调度器开始优化“队列、重试和吞吐”。
5. **framework 如何通向 workload-aware scheduling**  
   `v1.35-v1.36` 的 `Workload / PodGroup / gang / topology-aware / workload-aware preemption`。
...
- [Gang Scheduling](https://v1-35.docs.kubernetes.io/docs/concepts/scheduling-eviction/gang-scheduling/)
