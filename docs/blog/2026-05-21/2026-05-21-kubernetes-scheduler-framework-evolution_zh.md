...
source_urls:
  ...
  - https://raw.githubusercontent.com/kubernetes/kubernetes/master/staging/src/k8s.io/kube-scheduler/framework/interface.go
...
<p align="center">
  <img src="./kubernetes_scheduler_framework_with_preenqueue.svg" width="1200" alt="Kubernetes scheduling framework with PreEnqueue and QueueSort">
</p>

*基于 `KEP-624 Scheduling Framework` 原图风格扩展，补入 `PreEnqueue`、`Sort (QueueSort)` 与 gated Pod 路径。*

这张图最值得强调的，不是插件名字，而是 **调度器被分层了**：

- 队列前置阶段：`PreEnqueue`
- 队列排序入口：`Sort (QueueSort)`
...
### 2.3 如果按“完整 plugin 接口”来数，应该怎么列

如果从 **`k8s.io/kube-scheduler/framework/interface.go` 中定义的接口** 来看，完整清单不能只看图上的主流程，还要把队列侧、batching/workload-aware 侧的新接口一起算上。

下面这张图更适合当“完整接口索引”。它把 **main extension point interfaces** 和 **auxiliary interfaces** 放在一张 matrix 里，而且直接用全名展示接口，而不是只给缩写。

![Kubernetes scheduler in-tree plugin interface matrix](./kubernetes_scheduler_plugin_interface_matrix_full_names.png)

这张图里还有三条很值得直接拿来写进正文的注记：

- `PreBindPlugin` 包含 `PreBindPreFlight`
- `PreFilterExtensions`、`ScoreExtensions`、`PlacementScoreExtensions` 表示插件返回了对应扩展实现
- 当前没有 in-tree `PostBindPlugin` 实现，而 `DefaultPreemption` 还实现了 `PodGroupPostFilterPlugin`

按你现在这篇文章的语境，更合适的分法是三层。
...
这部分里，`PoB` 很容易漏掉，但在官方接口里它是独立的 `PostBindPlugin`。

**第二层：batching / workload-aware scheduling 新增的 `*Plugin` 接口**

- `SG`: `SignPlugin`
- `PG`: `PlacementGeneratePlugin`
- `PLS`: `PlacementScorePlugin`
- `PGPoF`: `PodGroupPostFilterPlugin`

这里有两个容易混淆的点：

- `SignPlugin` 不是传统 extension point 图里那种“主阶段插件”，它服务于 opportunistic batching 和 gang scheduling 优化。
- `PodGroupPostFilterPlugin` 也属于官方接口的一部分，只是它面向 PodGroup scheduling / workload-aware preemption，不在经典 `Pod` 调度图里出现。

**第三层：不是独立 plugin，但经常会被一起算进去的扩展接口 / 方法**

- `EQ`: `EnqueueExtensions`
- `PFE`: `PreFilterExtensions`
- `SE`: `ScoreExtensions`
- `PLSE`: `PlacementScoreExtensions`
- `PreBindPreFlight`: `PreBindPlugin` 上的方法，不是独立 `Plugin`

这里我建议在文里明确一句：

> `NormalizeScore` 和 `NormalizePlacementScore` 不是独立 plugin，而分别属于
> `ScoreExtensions` 和 `PlacementScoreExtensions`。

所以如果你说“完整 plugin 应该包括哪些”，一个更严格的说法其实是：

- **主 `Plugin` 接口**：`PE/QS/PF/F/PoF/PSc/S/Res/Perm/PB/B/PoB/SG/PG/PLS/PGPoF`
- **辅助扩展接口**：`EQ/PFE/SE/PLSE`
- **插件内方法但非独立 plugin**：`PreBindPreFlight`

### 2.4 为什么我建议把 `v1.20` 也算进“成型期”
