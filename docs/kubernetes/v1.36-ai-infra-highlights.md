...
- **Prioritized list (Stable, KEP-4816)**:
  lets workloads express ordered fallback preferences across heterogeneous
  accelerators.
- **Extended resource support (Beta, KEP-5004)**:
  gives `vendor.com/device`-style workloads a migration path onto DRA.
- **Partitionable devices (Beta, KEP-4815)**:
  enables sharing a single accelerator as logical slices.
- **Device taints and tolerations (Beta, KEP-5055)**:
  improve safe scheduling on problematic or reserved devices.
- **Device binding conditions (Beta, KEP-5007)**:
  avoids binding Pods before attachable or externally prepared devices are
  actually ready.
- **Resource health status (Beta, KEP-4680)**:
  exposes device health directly in Pod status.
- **ResourceClaim support for workloads (Alpha, KEP-5729)**:
  extends shared-claim semantics toward PodGroups and large distributed jobs.
- **Node allocatable resources (Alpha, KEP-5517)**:
  starts exploring DRA for CPU and memory placement, not only accelerators.
- **Resource pool status (Alpha, KEP-5677)**:
  adds point-in-time visibility into pool capacity and allocation state.

**AI infra impact**: better GPU/NIC sharing efficiency, safer day-2
operations, more realistic migration from Device Plugin, and an early path
toward unified placement semantics for standard compute resources.
