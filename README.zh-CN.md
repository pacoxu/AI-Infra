...
基础设施架构师的关键实践和习惯。该图表围绕三大核心支柱组织：
**Kubernetes 核心功力**、**AI 工作负载 & GPU** 和 **AI 平台架构**，
并由 **开源影响力**、**知识传播**、**架构视野**、**社区领导力** 和
**自我管理** 等基础实践支撑。

| 每周两晚读 scheduler 源码 | 梳理 DRA 资源模型图 | 走读 kubelet 节点流程 | 实践 vLLM 推理部署 | 在 K8s 上跑 TensorRT-LLM | 记录不同 GPU 型号差异 | 画在线推理平台架构 | 设计训练作业编排流程 | 考虑多租户隔离方案 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 多 CRI/CNI 组合实验 | **Kubernetes 核心功力** | 编写调度链路时序图 | 整理 HBM/NVLink/RDMA 笔记 | **AI 工作负载 & GPU** | 做 KV-cache 持久化实验 | 接入 Gateway API / Ingress | **AI 平台架构** | 接入监控与日志链路 |
| 跟进关键 KEP 实现 | 性能 Profiling 与优化 | 故障注入排查练习 | 对比单机/多机推理拓扑 | 建立 GPU 选型对照表 | 关注新模型与算子趋势 | 设计金丝雀发布流程 | 编写容量与成本模型 | 预演扩容与故障场景 |
| 锁定 2 个主战场项目 | 每周 triage issue/PR | 主动认领无人维护模块 | **Kubernetes 核心功力** | **AI 工作负载 & GPU** | **AI 平台架构** | 阅读 DDIA 并做读书笔记 | 分析云厂商 AI 平台架构 | 整理典型调度/扩缩模式 |
| 撰写/评审设计文档 | **开源影响力** | 推动至少 1 个 KEP | **开源影响力** | **Cloud Native AI Infra on Kubernetes Lead** | **架构视野** | 为项目撰写 ADR 记录决策 | **架构视野** | 练习从需求到系统蓝图 |
| 在 review 中给出高质量建议 | 维护项目贡献指南 | 帮助新人提交首个 PR | **社区领导力** | **知识传播** | **自我管理** | 思考 3 年演进路线图 | 对比不同技术栈 trade-off | 复盘每次失败的设计 |
| 在 SIG/WG 担任 note taker | 偶尔主持社区例会 | 组织小型专题工作组 | 每月发表一篇技术长文 | 整理 AI-Infra 仓库索引 | 把常见问题写成 FAQ 文档 | 制定季度 OKR 与学习计划 | 每天 2 小时深度工作时间 | 每周 3 次运动保持体能 |
| 一对一辅导新贡献者 | **社区领导力** | 在会议中主动总结结论 | 准备高质量分享 slide 模板 | **知识传播** | 积极投 KubeCon/KCD CFP | 保证 7-8 小时睡眠 | **自我管理** | 每周写一次个人复盘 |
| 推动跨公司协作任务 | 学会温和但清晰地反对 | 复盘每次沟通冲突 | 在 Slack/邮件列表深度回复 | 录制短视频或直播讲解 | 中英双语同步核心内容 | 控制信息输入与刷屏时间 | 预留家庭与休息时间 | 每年安排一次长假充电 |

**注意：** 此目标达成图由 AI 生成（GPT），尚未根据个人情况调整。
它作为云原生 AI 基础设施领域职业发展的参考框架。
